...
{ // determine unbalanced left/right child -> determine linear/zig-zag pattern
  if (root->bf > 1)
  { // unbalanced left child
    if (root->left->bf >= 0)
    {                           // right rotation
      return rightRotate(root); // not zig zag pattern
    }
    else
    {                                      // left-right rotation -> zig zag pattern
      root->left = leftRotate(root->left); // make it a linear pattern
      return rightRotate(root);            // then do single rotation
    }
  }
  else if (root->bf < -1)
  { // unbalanced right child
    if (root->right->bf <= 0)
    {
