...
void pop(){
	...
	if(strcmp(correct[i], head->name)==0){
		if(head==tail){
			free(head);
			head = tail = NULL;
		}else{
	...
	}else if(strcmp(correct[i], tail->name)==0){
		if(head==tail){
			free(head);
			head = tail = NULL;
		}else{
		...
		Node *curr = head;
		while(curr){
			if(strcmp(correct[i], curr->name)==0){
				Node *temp = curr->next;
				temp->prev = curr->prev;
				curr->prev->next = temp;
				free(curr);
...
int main(){
    ...
    scanf("%d", &x);
    while(x<5){
    	printf("Please Input the Aount of Words Again (Minimum = 5): ");
        scanf("%d", &x);
	}
	...
	int counter=10;
	printf("You Have 10 Lives, Now Guess The Alphabtes for the Words\n");
	char val[10]={NULL};
	while(counter!=0){
		if(index == 0) print(val);
		char a;
		scanf("%c", &a);
		system("cls");
		...
		if(vali==0){
			...
			puts("You already Guessed the Alphabets");
		}
		
		print(val);
		
		puts("Alphabets thats already been Guessed: ");
		for(int i=0; i<index; i++){
		printf("%c ", val[i]);
		}
